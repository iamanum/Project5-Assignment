...
# Generate a key for encryption from the passkey
def generate_key_from_passkey(passkey):
    hashed = hashlib.sha256(passkey.encode()).digest()
    return base64.urlsafe_b64encode(hashed[:32])

# Encrypt data using the passkey
def encrypt_data(text, passkey):
    key = generate_key_from_passkey(passkey)
    cipher = Fernet(key)
    return cipher.encrypt(text.encode()).decode()

# Decrypt data if the passkey is correct
...
# Change the current page based on selection
...
# Footer
st.markdown("---")
st.markdown("🔐 Secure Data Encryption System | Developed for Secure Data Handling")
